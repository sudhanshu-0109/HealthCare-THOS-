...
export const getSocket = () => {
  ...
  const isStandalone = localStorage.getItem('hc_standalone_mode') !== 'false' || token?.startsWith('standalone-');

  // In standalone frontend-only mode, return an in-memory mock socket
  if (isStandalone) {
    if (!socket || !socket._isMock) {
      socket = {
        _isMock: true,
        connected: true,
        active: true,
        auth: { token },
        on: (event, cb) => { return socket; },
        off: (event, cb) => { return socket; },
        emit: (event, data) => { return socket; },
        disconnect: () => {},
      };
    }
    return socket;
  ...
  // Reuse a live OR actively-reconnecting socket.
