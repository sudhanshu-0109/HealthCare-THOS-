...
function ConfirmPriceModal({ req, onClose, onSuccess }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-3xl shadow-2xl max-w-md w-full p-6 border border-slate-100">
        <div className="flex items-center justify-between mb-1">
          <h3 className="font-extrabold text-slate-900 text-lg tracking-tight">Confirm Lab Request</h3>
        ...
        <div className="space-y-2.5 mb-4">
          {(req.items || []).map((it, i) => (
            <div key={it.id || i} className="flex items-center justify-between">
              <span className="text-sm font-semibold text-slate-700">{it.testName}</span>
              <span className="text-sm font-extrabold text-slate-900">₹{Number(it.estimatedPrice || 0).toLocaleString('en-IN')}</span>
            </div>
          ))}
        </div>
        <div className="flex justify-between items-center bg-teal-50/70 border border-teal-100 rounded-2xl px-4 py-3 mb-5">
          <span className="text-xs font-extrabold text-teal-800 uppercase tracking-wider">Total Amount</span>
          <span className="text-lg font-black text-teal-900">₹{total.toLocaleString('en-IN')}</span>
        </div>
        <div className="flex gap-3">
          ...
          <button onClick={handleSubmit} disabled={loading}
            className="flex-1 py-2.5 bg-gradient-to-r from-teal-600 to-emerald-600 hover:from-teal-700 hover:to-emerald-700 disabled:opacity-50 text-white rounded-2xl text-xs font-bold flex items-center justify-center gap-2 shadow-sm">
...
function RequestCard({ req, activeTab, onAction, actionLoading, onConfirm, onUpload }) {
  ...
  const getAction = () => {
    switch (activeTab) {
      case 'pending': return { label: 'Confirm Lab Request', style: 'bg-gradient-to-r from-teal-600 to-emerald-600 hover:from-teal-700 hover:to-emerald-700 text-white shadow-sm', kind: 'confirm' };
      case 'collected': return { label: 'Mark Processing', style: 'bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white shadow-sm', kind: 'advance' };
      case 'processing': return { label: 'Upload Report', style: 'bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white shadow-sm', kind: 'upload' };
      default: return null;
    }
  };
  ...
  return (
    <div className="bg-white rounded-3xl border border-slate-200/80 p-5 shadow-2xs hover:shadow-md transition-all">
