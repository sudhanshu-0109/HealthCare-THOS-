...
function HomeTab() {
  ...
  useEffect(() => {
    ...
    dispatchService.getDriverState()
      .then((res) => {
        ...
        if (state.activeRequest) setActiveRequest(state.activeRequest);
      })
  ...
  useEffect(() => {
    ...
    watchIdRef.current = navigator.geolocation.watchPosition(
      (pos) => {
        dispatchService.updateLocation(pos.coords.latitude, pos.coords.longitude).catch(() => {});
      },
      () => {},
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-xs text-slate-400 flex items-center gap-1 mt-0.5">
                <MapPin className="w-3 h-3" />
                {activeRequest.patientLat != null && activeRequest.patientLng != null
                  ? `${Number(activeRequest.patientLat).toFixed(4)}, ${Number(activeRequest.patientLng).toFixed(4)}`
                  : 'Location shared'}
              </p>
            </div>
