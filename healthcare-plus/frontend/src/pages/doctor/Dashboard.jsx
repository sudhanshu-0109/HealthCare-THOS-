            ...
            {labTests.length === 0 ? (
              ...
              return (
                <button key={t.id} onClick={() => toggle(t)}
                  className={`w-full flex items-center gap-3 p-3 rounded-xl border text-left transition-colors ${sel ? 'border-cyan-500 bg-cyan-50' : 'border-slate-100 hover:bg-slate-50'}`}>
                  ...
                  <div className="flex-1">
                    ...
                    {t.sampleType && <p className="text-xs text-slate-400">{t.sampleType}</p>}
                  </div>
        ...
        <div className="bg-gradient-to-br from-cyan-600 to-teal-700 rounded-2xl p-5 text-white">
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p className="font-bold text-xl mt-0.5">T-{currentToken.tokenNumber}</p>
            </div>
          ...
          <div className="space-y-2">
            {waiting.map((token, i) => (
              <div key={token.id} className="bg-white rounded-xl border border-slate-100 p-3 flex items-center gap-3">
                <div className="w-9 h-9 bg-amber-100 rounded-xl flex items-center justify-center text-amber-700 text-xs font-bold flex-shrink-0">
                  T-{token.tokenNumber}
                </div>
          ...
          <div className="space-y-1.5">
            {completed.map((t) => (
              <div key={t.id} className="flex items-center gap-2 text-sm">
                ...
                <span className="text-slate-400 text-xs ml-auto">T-{t.tokenNumber}</span>
              </div>
            ))}
          </div>
        </div>
